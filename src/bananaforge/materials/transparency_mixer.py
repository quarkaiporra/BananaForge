...
class TransparencyColorMixer:
    ...
    def __init__(
        ...
        if not all(self.opacity_levels[i] <= self.opacity_levels[i+1] for i in range(2)):
    ...
    def calculate_layer_opacities(
        ...
        if target_layers > len(self.opacity_levels):
            ...
            step = (1.0 - self.opacity_levels[-1]) / (target_layers - len(self.opacity_levels))
            for i in range(target_layers - len(self.opacity_levels)):
                extended_levels.append(min(1.0, self.opacity_levels[-1] + (i + 1) * step))
    ...
    def apply_opacity_model_to_image(
        ...
        opacity_layers = torch.zeros(
            num_levels, channels, height, width,
            device=self.device, dtype=base_image.dtype
        )
    ...
    def compute_achievable_colors(
        ...
        for i, base_color in enumerate(filament_colors):
            achievable_colors.append({
                ...
                'opacity': 1.0,
                'layers': 1,
                'accuracy_metrics': self._calculate_color_accuracy_metrics(base_color, base_color)
            })
            ...
            for overlay_idx, overlay_color in enumerate(filament_colors):
                ...
                for opacity in self.opacity_levels:
                    ...
                    achievable_colors.append({
                        ...
                        'opacity': opacity,
                        'layers': layer_count,
                        'accuracy_metrics': self._calculate_color_accuracy_metrics(mixed_color, overlay_color)
                    })
    ...
    def map_rgb_to_layer_combination(
        ...
        """
        achievable_colors = self.compute_achievable_colors(available_filaments, max_layers)
        ...
        return {
            ...
            'layer_recipe': self._create_layer_recipe(best_match) if best_match else None
        }
    ...
    def analyze_palette_expansion(
        ...
        for base_idx, base_color in enumerate(filament_colors):
            ...
            if len(unique_colors) > 1:
                ...
                color_range = torch.max(color_tensor, dim=0)[0] - torch.min(color_tensor, dim=0)[0]
    ...
    def compute_combinations_with_accuracy(
        ...
        for combo in combinations:
            ...
            combo['accuracy_metrics'] = {
                ...
                'perceptual_error': self._calculate_perceptual_error(achieved_color, overlay_color),
                'color_gamut_coverage': self._calculate_gamut_coverage(achieved_color, filament_colors)
            }
    ...
    def _calculate_color_accuracy_metrics(
        ...
        return {
            ...
            'perceptual_error': self._calculate_perceptual_error(achieved_color, target_color),
    ...
    def _calculate_gamut_coverage(
        self,
        color: torch.Tensor,
        reference_colors: List[torch.Tensor]
    ) -> float:
        ...
        coverage = torch.mean(torch.clamp(normalized_pos, 0, 1) * (1 - torch.clamp(normalized_pos, 0, 1))).item()
