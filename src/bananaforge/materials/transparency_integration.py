...
class TransparencyIntegration:
    ...
    def enable_transparency_mode(
        ...
        try:
            ...
            self.feature_status.update({
                'transparency_enabled': transparency_config.get('opacity_levels') is not None,
                'gradient_mixing_enabled': transparency_config.get('enable_gradient_mixing', False),
                'base_optimization_enabled': transparency_config.get('enable_base_layer_optimization', False)
            })
    ...
    def run_standard_optimization(
        ...
        backward_compatibility = self._assess_backward_compatibility(result, transparency_enabled)
    ...
    def test_cli_integration(
        ...
        try:
            ...
            # Check for conflicts
            arg_conflicts = self._check_argument_conflicts(existing_args, transparency_args)
    ...
    def _create_transparency_assignments(
        ...
        transparency_assignments = torch.zeros_like(material_assignments, dtype=torch.float32)
    ...
    def _run_transparency_enhanced_optimization(
        self, image: torch.Tensor, height_map: torch.Tensor, 
        material_assignments: torch.Tensor, materials: List[Dict],
        ...
        final_assignments = optimization_result.get('optimized_assignments', material_assignments)
    ...
    def _run_standard_optimization_legacy(
        ...
        final_assignments = material_assignments.clone() if material_assignments is not None else None
    ...
    def _run_configured_transparency_optimization(
        ...
        if transparency_config.get('transparency_enabled', False):
            ...
            if height_map is not None and material_assignments is not None and materials:
                transparency_result = self.transparency_optimizer.optimize_with_transparency(
                    height_map, material_assignments, materials
                )
    ...
    def _assess_backward_compatibility(self, result: Dict, transparency_enabled: bool) -> Dict:
    ...
    def _check_argument_conflicts(self, existing_args: List[str], new_args: List[str]) -> bool:
    ...
    def _simulate_command_execution(self, parsed_args: Dict) -> Dict:
        ...
        return {
            'success': success,
            'outputs_generated': len(outputs_generated) > 0
        }