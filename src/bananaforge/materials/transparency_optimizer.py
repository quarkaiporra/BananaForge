...
class TransparencyOptimizer:
    ...
    def optimize_with_transparency(
        ...
        optimized_swaps = self.calculate_material_swaps(optimized_assignments, 'standard')
    ...
    def calculate_cost_savings(
        ...
        material_usage_reduction = self._calculate_material_usage_reduction(
            baseline_assignments, optimized_assignments if transparency_enabled else baseline_assignments,
            materials
        )
    ...
    def optimize_for_quality_preservation(
        ...
        return {
            ...
            'quality_threshold_met': best_quality['visual_quality_score'] >= quality_threshold
        }
    ...
    def generate_detailed_savings_report(
        ...
        if include_material_analysis:
            material_savings = self._calculate_detailed_material_savings(
                baseline_assignments, optimization_result['optimized_assignments'], materials
            )
            report['material_savings'] = material_savings
            report['summary']['total_material_savings'] = material_savings['material_saved_grams']
    ...
    def optimize_with_constraints(
        ...
        return {
            ...
            'constraint_violations': list(set(constraint_violations)) if constraint_violations else []
        }
    ...
    def optimize_large_model(
        ...
        chunks = self._divide_into_chunks(
            height_map, material_assignments, chunk_size
        )
        ...
        swap_reduction = ((baseline_swaps - optimized_swaps) / max(baseline_swaps, 1)) if baseline_swaps > 0 else 0
    ...
    def _calculate_material_costs(
        ...
        for mat_idx, material in enumerate(materials):
            ...
            volume_cm3 = material_pixels * pixel_area * layer_height / 10.0  # Convert to cm³
        ...
        estimated_print_time_hours = material_assignments.shape[0] * layer_height / 10.0  # Simplified
    ...
    def _apply_transparency_strategy(
        ...
        if base_strategy == 'single_dark':
            ...
            base_indices = [info['base_color_index'] for info in base_opt_result['selected_base_colors']]
    ...
    def _calculate_time_savings(
        ...
        baseline_print_time = baseline_assignments.shape[0] * 0.08 / 10.0 * 60  # Layer time estimate in minutes
        ...
        return {
            ...
            'optimized_print_time': optimized_print_time + optimized_swaps * time_per_swap_minutes,
    ...
    def _calculate_detailed_material_savings(
        ...
        baseline_usage = self._calculate_total_material_usage(baseline_assignments, materials)
        optimized_usage = self._calculate_total_material_usage(optimized_assignments, materials)
        
        material_saved = baseline_usage - optimized_usage
        waste_reduction = self._calculate_waste_reduction(baseline_assignments, optimized_assignments)
    ...
    def _count_unique_swaps(self, current_layer: torch.Tensor, next_layer: torch.Tensor) -> int:
        """Count unique material swaps between layers."""
        unique_changes = torch.unique(torch.stack([current_layer, next_layer], dim=0), dim=0)
        return len(unique_changes) - 1  # Subtract one for no-change case
    
    def _calculate_swap_complexity_weight(self, current_layer: torch.Tensor, next_layer: torch.Tensor) -> float:
    ...
    def _divide_into_chunks(
        self, height_map: torch.Tensor, material_assignments: torch.Tensor, chunk_size: int
    ) -> List[Dict]:
    ...
    def _process_chunks_parallel(self, chunks: List[Dict], materials: List[Dict]) -> List[Dict]:
        """Process chunks in parallel."""
    ...
    def _process_chunks_sequential(self, chunks: List[Dict], materials: List[Dict]) -> List[Dict]:
        """Process chunks sequentially."""
    ...
    def _select_contrasting_bases(self, base_colors: List[torch.Tensor], num_bases: int) -> List[int]:
    ...
    def _apply_layer_transparency(
        ...
        if len(unique_materials) > 2:  # If too many materials, apply transparency
            ...
            material_counts = [(mat.item(), torch.sum(current_layer == mat).item()) for mat in unique_materials]
    ...
    def _count_color_transitions(self, material_assignments: torch.Tensor) -> int:
        ...
        for layer in range(material_assignments.shape[0] - 1):
            ...
            unique_pairs = torch.unique(torch.stack([current.flatten(), next_layer.flatten()]), dim=1)
